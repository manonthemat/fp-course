...
filter ::
  ...
  -> List a
filter f =
  foldRight (\x y -> if f x == True then x :. y else y) Nil
...
seqOptional x
  | elem Empty x = Empty
  | otherwise    = Full (foldRight (\a b -> (unpack a) :. b) Nil x)
  where
    unpack (Full x) = x
...
find ::
  ...
  -> Optional a
find _ Nil      = Empty
find f (x :. y) = if f x then Full x else find f y
